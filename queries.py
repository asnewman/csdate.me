...
class Database:
   ...
   def authCheck(self, username, password):
      ...
      query = "SELECT id FROM Users WHERE username = '" + username + "' AND password = '" + passw + "';"
   ...
   def addUser(self, name, email, password, salt):
      self.cur.execute("START TRANSACTION;")
      cmd = "INSERT INTO Users VALUES (NULL, '" + name + "', '" + password + "', '" + email + "', NULL, NULL, '" + salt + "');"
      self.cur.execute(cmd)
      cmd2 = "INSERT INTO Questions VALUES (NULL, FALSE, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'default.png');"
   ...
   def setQuestions(self, userid, fName, mName, lName, gender, state, city, birthday, favoriteOS, phoneOS, relationship, gaming, favLang1, favLang2, favLang3, favHobby1, favHobby2, favHobby3, wpm):
      cmd = "UPDATE Questions SET completed = true, firstName = '" + fName + "', middleName = '" + mName + "', lastName = '" + lName +  "', gender = '" + gender + "', state = '" + state + "', city = '" + city + "', birthday = '" + birthday + "', favoriteOS = '" + favoriteOS + "', phoneOS = '" + phoneOS + "', relationship = '" + relationship + "', gaming = '" + gaming + "', favLang1 = '" + favLang1 + "', favLang2 = '" + favLang2 + "', favLang3 = '" + favLang3 + "',  favHobby1 = '" + favHobby1 + "',  favHobby2 = '" + favHobby2 + "', favHobby3 = '" + favHobby3 + "', wpm = " + wpm + " WHERE id = " + str(userid) + ";"
      self.cur.execute("START TRANSACTION;")
      self.cur.execute(cmd)
      self.cur.execute("COMMIT;")
   ...
   def updateQuestions(self, userId, fName, mName, lName, gender, state, city, birthday, favoriteOS, phoneOS, relationship, gaming, favLang1, favLang2, favLang3, favHobby1, favHobby2, favHobby3, wpm):
      ...
      if fName:
         valid = True
         cmd = cmd + " firstName = '" + fName + "',"
      if mName:
         valid = True
         cmd = cmd + " middleName = '" + mName + "',"
      if lName:
         valid = True
         cmd = cmd + " lastName = '" + lName + "'," 
      if gender:
         valid = True
      ...
      if favLang1:
         valid = True
         cmd = cmd + " favLang1 = '" + favLang1 + "',"
      if favLang2:
         valid = True
         cmd = cmd + " favLang2 = '" + favLang2 + "',"
      if favLang3:
         valid = True
         cmd = cmd + " favLang3 = '" + favLang3 + "',"
      if favHobby1:
         valid = True
         cmd = cmd + " favHobby1 = '" + favHobby1 + "',"  
      if favHobby2:
         valid = True
         cmd = cmd + " favHobby2 = '" + favHobby2 + "'," 
      if favHobby3:
         valid = True
         cmd = cmd + " favHobby3 = '" + favHobby3 + "'," 
      if wpm:
         valid = True
         cmd = cmd + " wpm = " + wpm + "," 
   ...
   def singleSearch(self, search, attribute, userid):
      if (attribute == "favLang" or attribute == "favHobby"):
         ...
         second = attribute + "2"
         third = attribute + "3"
         cmd = "Select * from Questions WHERE (" + first + " = '" + search + "' OR " + second + " = '" + search + "' OR " + third + " = '" + search + "') AND id != "+ str(userid) +  ";"
      elif (attribute == "wpm"):
         if('-' in search):
            nums = search.split('-')
         ...
         cmd = "Select * from Questions WHERE " + attribute + " = '" + search + "' AND id != " + str(userid) + " ;"
      self.cur.execute(cmd)
      res = self.cur.fetchall()
      tuples = []
      for result in res:
   ...
   def indepthSearch(self, require, fName, mName, lName, gender, state, city, favoriteOS, phoneOS, relationship, gaming, favLang1, favLang2, favLang3, wpm):
      ...
      atLeastOne = 0
      if (len(fName) > 0):
         self.fName = require + " firstName = '" + fName + "' " 
         atLeastOne = 1
      if (len(mName) > 0):
         self.mName = require + " middleName = '" + mName + "' "
         atLeastOne = 1
      if (len(lName) > 0):
         self.lName = require + " lastName = '" + lName + "' " 
         atLeastOne = 1
      ...
      if (len(gaming) > 0 and gaming != "none"):
         ...
         atLeastOne = 1
      if (len(favLang1) > 0 ):
         self.favLang1 = require + " (favLang1 = '" + favLang1 + "' OR favLang2 = '" + favLang1 + "' OR favLang3 = '" + favLang1 + "') "
         atLeastOne = 1

      if (len(favLang2) > 0 ):
         self.favLang2 = require + " (favLang2 = '" + favLang2 +  "' OR favLang2 = '" + favLang2 + "' OR favLang3 = '" + favLang2 + "') "
         atLeastOne = 1
   ...
   def sortProfiles(self, userid):
      ...
      for (uid, score) in scores:
         self.cur.execute("SELECT * FROM Questions WHERE id = " + str(uid) + ";")
         res = self.cur.fetchall()
         tuples.append(res[0])
      return tuples

   def userProfile(self, userid):
      self.cur.execute("SELECT * FROM Questions WHERE id = " + str(userid) + ";")
      res = self.cur.fetchall()
      return res[0]
